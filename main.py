...
# --- Conversation States ---
TASK_DESC, TASK_LINK, TASK_REWARD, REJECT_REASON, BROADCAST_MESSAGE, ANNOUNCEMENT_TEXT, NEW_CODE, NEW_CODE_REWARD, NEW_CODE_USES, SUBMIT_TASK_REJECT_REASON = range(10)
...
broadcast_conv = ConversationHandler(entry_points=[CallbackQueryHandler(broadcast_start, pattern="^admin_broadcast$")], states={BROADCAST_MESSAGE: [MessageHandler(filters.ALL & ~filters.COMMAND, broadcast_message)]}, fallbacks=[], per_user=True)
announcement_conv = ConversationHandler(entry_points=[CallbackQueryHandler(announcement_start, pattern="^admin_set_announcement$")], states={ANNOUNCEMENT_TEXT: [MessageHandler(filters.TEXT & ~filters.COMMAND, set_announcement_text)]}, fallbacks=[], per_user=True)
