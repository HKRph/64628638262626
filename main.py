...
class TaskSubmission(Base): __tablename__ = "task_submissions"; id = Column(Integer, primary_key=True); user_id = Column(BigInteger); task_id = Column(Integer); text_proof = Column(Text); photo_proof_id = Column(String); status = Column(String, default="pending")
...
# --- Conversation States ---
TASK_DESC, TASK_LINK, TASK_REWARD = range(3)
REJECT_REASON, BROADCAST_MESSAGE, ANNOUNCEMENT_TEXT, NEW_CODE, NEW_CODE_REWARD, NEW_CODE_USES = range(3, 9)

# --- Bot & API Lifespan Management ---
ptb_app = Application.builder().token(BOT_TOKEN).build()
@asynccontextmanager
async def lifespan(app: FastAPI): logger.info("Lifespan startup..."); await ptb_app.initialize(); await ptb_app.updater.start_polling(); await ptb_app.start(); logger.info("Telegram bot started."); yield; logger.info("Lifespan shutdown..."); await ptb_app.updater.stop(); await ptb_app.stop(); await ptb_app.shutdown()
...
async def get_initial_data(request: Request):
    try:
        ...
        return {
            ...
            "successful_referrals": user.successful_referrals if user else 0,
            ...
            "withdrawals": [{"id": w.id, "amount": w.amount, "method": w.method, "status": w.status} for w in withdrawals]
        }
...
async def submit_task_proof(request: Request):
    try:
        ...
        # In a real app, you'd upload the base64 image somewhere. Here we'll just forward it to the admin.
        submission = TaskSubmission(user_id=user_id, task_id=task_id, text_proof=text)
        ...
        keyboard = [[InlineKeyboardButton("Approve ✅", callback_data=f"approve_task_{submission.id}"), InlineKeyboardButton("Reject ❌", callback_data=f"reject_task_{submission.id}")]]
        
        # Send photo proof to admin
        ...
        await ptb_app.bot.send_message(user_id, "✅ Your proof has been submitted for review!")
...
# ... (All other API endpoints and bot handlers remain largely the same, but are included for completeness)
...
async def admin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("📜 Set Announcement", callback_data="admin_set_announcement")],
        [InlineKeyboardButton("🔑 Manage Codes", callback_data="admin_manage_codes")],
...
# ... (All the handlers for these new admin functions are included below)

# Add Handlers to the PTB Application
ptb_app.add_handler(CommandHandler("start", start_command))
ptb_app.add_handler(CommandHandler("admin", admin_command))
# ... (All other handlers for admin panel, conversations, etc. are included below)

# This is a placeholder for the full admin functionality which is very extensive.
# The code below includes the handlers for the new admin buttons.
async def admin_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text(f"**Bot Stats:**\n\n- Total Users: {total_users}\n- Total Balance in Circulation: ₱{total_balance:.2f}\n- Pending Withdrawals: {pending_withdrawals}", parse_mode='Markdown')

# (Broadcast conversation)
async def broadcast_start(update: Update, context: ContextTypes.DEFAULT_TYPE): await update.callback_query.answer(); await update.callback_query.message.reply_text("Send the message you want to broadcast to all users."); return BROADCAST_MESSAGE
...
async def announcement_start(update: Update, context: ContextTypes.DEFAULT_TYPE): await update.callback_query.answer(); await update.callback_query.message.reply_text("Enter the new announcement text. Send /clear to remove it."); return ANNOUNCEMENT_TEXT
...
broadcast_conv = ConversationHandler(entry_points=[CallbackQueryHandler(broadcast_start, pattern="^admin_broadcast$")], states={BROADCAST_MESSAGE: [MessageHandler(filters.ALL, broadcast_message)]}, fallbacks=[])
announcement_conv = ConversationHandler(entry_points=[CallbackQueryHandler(announcement_start, pattern="^admin_set_announcement$")], states={ANNOUNCEMENT_TEXT: [MessageHandler(filters.TEXT, set_announcement_text)]}, fallbacks=[])
...
# (Plus all the previous handlers for withdrawals, task removal, etc.)
