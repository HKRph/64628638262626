...
SUBMIT_TASK_REJECT_REASON = range(9, 10)
...
# ... (Withdrawal endpoint is omitted for brevity as it's unchanged)

# --- Telegram Bot Handlers (must be defined before use) ---
...
async def get_task_reward(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try: context.user_data['task_reward'] = float(update.message.text); await update.message.reply_text("This task is now set. Use /admin to manage."); return ConversationHandler.END
    except ValueError: await update.message.reply_text("Invalid amount."); return TASK_REWARD

# ... (All other admin and helper functions must also be defined here)
...
add_task_conv = ConversationHandler(
    entry_points=[CallbackQueryHandler(add_task_start, pattern="^admin_manage_tasks$")],
    states={
